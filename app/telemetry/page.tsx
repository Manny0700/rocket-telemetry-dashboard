...
import { clearFlightLog } from "@/lib/flightLogger";
...
export default function TelemetryPage() {
  // Local state mirrors the global store — updated via listener
  ...
  useEffect(() => {
    ...
    ensureWebSocket(); // no-op if already running
    return () => { store.listeners.delete(listener); };
  }, []);

  // MET timer — continues from where it left off
  useEffect(() => {
  ...
  // Auto-locate only once on first mount (if not already set)
  useEffect(() => {
    if (store.gsLat === null) locateGroundStation();
  }, []);

  const { gsLat, gsLon, gsAccuracy, lat, lon, rssi, connected,
          altitudeData, velocityData, labels, currentAlt, currentVel,
          rawPacket, payloads } = store;

  const distanceMi = gsLat && gsLon && lat && lon
    ...
    <div className="container" style={{ position: "relative" }}>
      ...
      <div className="card" style={{ marginTop: "40px", fontFamily: "monospace" }}>
        ...
        {rawPacket ? (
          ...
          <p style={{ color: "rgba(255,255,255,0.3)", marginTop: "12px" }}>Waiting for telemetry packets...</p>
        )}
      </div>
